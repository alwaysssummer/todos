...
export function useTextbooks() {
    ...
    // 교재 로컬 경로 변경
    const updateTextbookLocalPath = useCallback(async (id: string, localPath: string | null): Promise<Textbook> => {
        try {
            const { data, error } = await supabase
                .from('textbooks')
            ...
            console.error('Error updating textbook local path:', error)
            throw error
        }
    }, [])

    // 교재 메모 변경
    const updateTextbookMemo = useCallback(async (id: string, memo: string | null): Promise<Textbook> => {
        try {
            const { data, error } = await supabase
                .from('textbooks')
                .update({ memo })
            ...
            console.error('Error updating textbook memo:', error)
            throw error
        }
    }, [])
